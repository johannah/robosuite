...
from robosuite.utils.input_utils import input2action, absinput2action
...
if __name__ == "__main__":
    ...
    parser.add_argument("--robots", nargs="+", type=str, default="Jaco", help="Which robot(s) to use in the env")
    ...
    parser.add_argument("--device", type=str, default="mimic")
    ...
    elif args.device.lower()  == 'mimic':
        from robosuite.devices import Mimic
        device = Mimic()
    else:
        raise Exception(
    ...
    while True:
        ...
        obs = env.reset()
        initial_ee_pos = pos = obs['robot0_eef_pos']
        abs_robot_shoulder = np.array([-.2, 0.0, 1.2]) # abs position of shoulder 
        rel_robot_shoulder = np.array([-.2, 0.0, .2]) # abs position of shoulder 
        touch_shoulder_action = abs_robot_shoulder - initial_ee_pos  
